...
def main():
    p=argparse.ArgumentParser();p.add_argument('--source',required=True);p.add_argument('--run',default='runs/final');p.add_argument('--deadline',type=float,required=True);p.add_argument('--simulations',type=int,default=128);p.add_argument('--lr',type=float,default=.001);p.add_argument('--cuda-graph-batch',type=int,default=0);a=p.parse_args()
    ...
    while time.time()<a.deadline:
        ...
        attempt+=1
        command=[sys.executable,'-u','-m','training.train','--run',str(run),'--resume',str(resume),'--games','128','--games-per-iteration','64','--updates','64','--simulations',str(a.simulations),'--batch-size','256','--lr',str(a.lr),'--seconds',str(max(1,int(a.deadline-time.time()))),'--deadline',str(a.deadline),'--max-ply','300','--cuda-graph-batch',str(a.cuda_graph_batch)]
