...
def main():
    p=argparse.ArgumentParser();p.add_argument('--source',required=True);p.add_argument('--run',default='runs/final');p.add_argument('--deadline',type=float,required=True);p.add_argument('--simulations',type=int,default=128);p.add_argument('--lr',type=float,default=.001);p.add_argument('--native-forest',action='store_true');p.add_argument('--games',type=int,default=128);p.add_argument('--cuda-graph-batch',type=int,default=0);a=p.parse_args()
    ...
    while time.time()<a.deadline:
        ...
        if a.native_forest:command.append('--native-forest')
