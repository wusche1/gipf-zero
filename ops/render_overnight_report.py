...
def render(summary_path: Path) -> Path:
    ...
    lines += ["", "## Protocol", "", f"- Training: isolated **{config.get('training_seconds', 600)} seconds per run**, {len(seeds)} seed(s), native forest search: **{bool((config.get('selfplay') or {}).get('native_forest', False))}**.", f"- Equal-CPU duels: {duel_cfg.get('seconds_cpu', '—')} seconds per match; fixed-simulation duels: {duel_cfg.get('seconds_sim', '—')} seconds per match.", "- Parameter counts are read from each run's initial `start` metric, before training."]
    if len(heads) > 1:
        lines.append("- This is **not a pure architecture ablation**: the compared models use different output heads (" + ", ".join(sorted(heads)) + ").")
    lines += ["- Results are engine-baseline evidence only; this report makes no human-rating claim.", "", "## Training", ""]
